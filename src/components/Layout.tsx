...
export default function Layout({ children, theme, toggleTheme, tools, onFileDrop, viewMode }: LayoutProps) {
  ...
  // Define which routes should show the bottom nav (mobile app mode)
  const mobileNavRoutes = ['/', '/students', '/teachers', '/settings', '/mcq-test']
  const isMobileRoute = mobileNavRoutes.includes(location.pathname)
  const showMobileNav = (isNative || viewMode === 'android') && !location.pathname.includes('routine-scrapper')
  ...
  // Show nav on mobile routes, but NOT when viewing PDF tools (tools have their own back navigation)
  const activeTool = tools.find(t => {
    const pathPart = t.title.split(' ')[0].toLowerCase()
    return location.pathname.includes(`/${pathPart}`)
  })
  
  const shouldShowNav = showMobileNav && isMobileRoute && !activeTool
  
  // DEBUG: Log navigation state
  console.log('[DEBUG Layout] pathname:', location.pathname, 'shouldShowNav:', shouldShowNav)
  ...
  const handleFabClick = () => {
    hapticImpact()
    console.log('[FAB] Clicked, pathname:', location.pathname)
    
    // Dispatch event based on current route
    if (location.pathname === '/students') {
      console.log('[FAB] Dispatching add-student')
      window.dispatchEvent(new CustomEvent('add-student'))
    } else if (location.pathname === '/teachers') {
      console.log('[FAB] Dispatching add-teacher')
      window.dispatchEvent(new CustomEvent('add-teacher'))
    } else {
      // Default to add-routine for home and any other route
      console.log('[FAB] Dispatching add-routine (default)')
      window.dispatchEvent(new CustomEvent('add-routine'))
    }
  }
        ...
        <footer className="border-t border-gray-100 dark:border-white/5 mt-20 bg-white dark:bg-black relative z-10">
          <div className="max-w-7xl mx-auto px-6 md:px-8 py-10 md:py-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-12 gap-8 mb-12">
              ...
              <div className="col-span-2 md:col-span-6 space-y-4">
                <Link to="/" className="flex items-center gap-2.5 text-gray-900 dark:text-white group w-fit">
                  ...
                  <span className="font-bold tracking-tight text-lg group-hover:text-rose-500 transition-colors">MCQ Test</span>
                </Link>
                <p className="text-gray-500 dark:text-zinc-500 text-xs leading-relaxed max-w-sm">
                  Online MCQ examination platform for students. <br/>
                  Track attendance, manage classes, and take tests.
                </p>
                <div className="flex items-center gap-2 pt-1">
                   ...
                   <a href="https://github.com/potatameister" target="_blank" className="p-2 bg-gray-50 dark:bg-zinc-900 rounded-xl hover:bg-rose-500 hover:text-white transition-all text-gray-500 dark:text-zinc-500">
                      <GHIcon size={14} />
                   </a>
                </div>
              </div>
              ...
              <div className="col-span-1 md:col-span-3">
                <h4 className="font-bold text-[10px] uppercase tracking-widest text-gray-900 dark:text-white mb-4">Platform</h4>
                <ul className="space-y-2.5 text-xs text-gray-500 dark:text-zinc-500">

                  <li><Link to="/privacy" className="hover:text-rose-500 transition-colors">Privacy Policy</Link></li>
                  <li><Link to="/mcq-test" className="hover:text-rose-500 transition-colors">Take Test</Link></li>
                </ul>
              </div>

              {/* Community Column */}
              <div className="col-span-1 md:col-span-3">
                <h4 className="font-bold text-[10px] uppercase tracking-widest text-gray-900 dark:text-white mb-4">Support</h4>
                <ul className="space-y-2.5 text-xs text-gray-500 dark:text-zinc-500">
                  ...
                  <li><a href="https://github.com/potatameister" target="_blank" className="hover:text-rose-500 transition-colors">Report Issue</a></li>
                  <li><Link to="/thanks" className="hover:text-rose-500 transition-colors">Contributors</Link></li>
                </ul>
              </div>
            ...
            <div className="pt-6 border-t border-gray-100 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-[11px] text-gray-400 dark:text-zinc-600 font-medium">
               <p>© 2026 MCQ Test Platform. All rights reserved.</p>
